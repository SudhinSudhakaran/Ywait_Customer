...
const PreviousAppointmentList = () => {
    ...
    return (
      <View
        ...
        <Text
          ...
          {t(Translations.NO_PREVIOUS_DATA_AVAILABLE)}
        </Text>
        <Text
          style={{
            alignSelf: 'center',
  ...
  const getUpcomingList = (isLoaderRequired, pageNumber) => {
    ...
    DataManager.getPreviousList(pageNumber).then(
  ...
  const renderItem = ({item, index}) => {
    return <PreviousDataCell item={item} index={index} />;
  };

  const PreviousDataCell = ({item}) => {
    var departmentName = '';
      ...
      <TouchableOpacity
        ...
        <View
          ...
          <GetImage
            ...
            fullName={(item?.servingUser_id?.fullName || 'N/A').trim()}
            alphabetColor={Colors.SECONDARY_COLOR}
            url={item?.servingUser_id?.image}
          />
        </View>
        ...
        <View style={{marginBottom: 20, flex: 1}}>
          ...
          <Text
            ...
            numberOfLines={1}>
            {item?.servingUser_id?.name || Globals.BUSINESS_DETAILS.name}
          </Text>
          {/* DEPARTMENT */}
          {showDepartmentLabel ? (
...
export default React.memo(PreviousAppointmentList);
